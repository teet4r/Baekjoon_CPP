...
void number::get_primes(int max_integer, vector<int> &container) {
	if (max_integer < 2) {
		return;
	}
...
void number::get_divisors(int n, vector<int> &container) {
	if (n < 1) {
		return;
	}
...
double number::get_power(long long base, long long exponent, long long modulo) {
	long long result = 1;
...
bool number::miller_rabin_primality_test(long long n, long long a) {
	if (n < 2 || a < 2) {
		return false;
	}
	if (!(a % n)) {
...
bool number::is_maybe_prime(int n) {
...
bool number::is_maybe_prime(long long n) {
